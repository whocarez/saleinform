...
class Demands_model extends Docmodel{
	...
	public function create_record(){
		...
		$ins_tours = array('_touroperators_rid'=>$this->ci->input->post('_touroperators_rid'),
								...
								'hotel_name'=>$this->ci->input->post('hotel_name'),
	...
	public function GetOrdererMore($p_ordererRid){
		$this->db->select('_clients.*, _cities.city_name');
		$this->db->from('_clients');
		$this->db->join('_cities', '_cities.rid = _clients._cities_rid', 'LEFT');
		$this->db->where(array('_clients.rid'=>$p_ordererRid));
		$query = $this->db->get();
		return $query->row();	
	}
	...
	public function SetPayDate($docRid){
		$this->db->trans_begin();
		$yesterday = mktime(0,0,0,date('m'),date('d')-1,date('Y'));
		$ddd = date('d.m.Y',$yesterday);
		$updateHeaderArray = array('pay_date'=>$this->toMySQLDate($ddd), 'correct_date'=>$this->toMySQLDate($ddd));
		$this->db->update('_demands_headers', $updateHeaderArray, array('_demands_headers._documents_rid'=>$docRid));		
		if ($this->db->trans_status() === FALSE){
    		$this->db->trans_rollback();
    		return False;
		}else{
	...
	public function SetAvanceDate($docRid){
		$this->db->trans_begin();
		#$yesterday = mktime(0,0,0,date('m'),date('d')-1,date('Y'));
		$yesterday = mktime(0,0,0,date('m'),date('d'),date('Y'));
		$ddd = date('d.m.Y',$yesterday);
		$this->db->select('summ_pay');
		$this->db->from('_demands_headers');
		$this->db->where(array('_demands_headers._documents_rid'=>$docRid));
		$query = $this->db->get();
		$summ_pay = $query->row()->summ_pay;
		$updateHeaderArray = array('avance_date'=>$this->toMySQLDate($ddd), 'avance_summ'=>$summ_pay);
		$this->db->update('_demands_headers', $updateHeaderArray, array('_demands_headers._documents_rid'=>$docRid));		
		if ($this->db->trans_status() === FALSE){
    		$this->db->trans_rollback();
    		return False;
	...
	public function SetGaranted($docRid, $garanted=1){
		$this->db->where(array('_documents_rid'=>$docRid));
		$this->db->update('_demands_headers', array('is_garanted'=>$garanted));
		if ($this->db->trans_status() === FALSE){
    		$this->db->trans_rollback();
    		return False;
		}else{
	...
	public function MoveRecord(){
		$this->db->trans_begin();
		$updateArr = array('owner_users_rid'=>$_POST['manager']);
		$this->db->update('_documents', $updateArr, array('rid'=>$this->ciObject->GetSessionData('activerecord')));
		$this->db->update('_demands_headers', $updateArr, array('_documents_rid'=>$this->ciObject->GetSessionData('activerecord')));
		$this->db->select('rid');
		$this->db->from('_demands_headers');
		$this->db->where(array('_documents_rid'=>$this->ciObject->GetSessionData('activerecord')));
		$query = $this->db->get();
		$headerRid = $query->row()->rid;
		$this->db->update('_demands_rows', $updateArr, array('_demands_headers_rid'=>$headerRid));
		
		$this->db->select('_clients_rid');
		$this->db->from('_demands_rows');
		$this->db->where(array('_demands_headers_rid'=>$headerRid));
		$query = $this->db->get();
		if($query->num_rows()){
			foreach($query->result() as $row){
				$this->db->update('_clients', $updateArr, array('rid'=>$row->_clients_rid));
			}
		}
		if ($this->db->trans_status() === FALSE){
    		$this->db->trans_rollback();
