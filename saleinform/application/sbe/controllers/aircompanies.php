...
class Aircompanies extends Crmcontroller {
	...
	private function find(){
		...
		if ($this->form_validation->run() == True){
			$search_rule = array();
			if($this->input->post('code')) $search_rule['_aircompanies.code'] = $this->input->post('code');
			if($this->input->post('name')) $search_rule['_aircompanies.name'] = $this->input->post('name');
			if($this->input->post('iata')) $search_rule['_aircompanies.iata'] = $this->input->post('iata');
	...
	private function move(){
		...
		$data['content'] = $this->load->view('aircompanies/move', $data, True);
		return $this->load->view('layouts/main_layout', $data);
	}
	...
	private function vcreate(){
		...
		if ($this->form_validation->run() === True){
			if($rid = $this->aircompanies_model->create_record()){
				$this->session->set_flashdata('success', True);
				redirect(get_currcontroller()."/vedit/$rid/mrid/".get_curririd(), 'refresh');
				return;
			}
			else {
				$data['success'] = false;
	...
	private function vmove(){
		...
		$data['content'] = $this->load->view('aircompanies/vmove', $data, True);
		return $this->load->view('layouts/valuepicker_layout', $data);
	}
