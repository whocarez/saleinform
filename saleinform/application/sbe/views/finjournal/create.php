<div class="grid">
<h3><?=$title?></h3>
<?= form_open(get_currcontroller()."/create/{$doc_rid}/mrid/".get_curririd(), array('id'=>'create_'.$orid, 'autocomplete'=>'off'))?>
...
<div class="column span-8 last">
	<?=form_input('oper_date', date_conv(set_value('oper_date', '')), 'id="oper_date" class="text" readonly="readonly" style="width:90px;"')?>
	<script type="text/javascript">
		$('#oper_date').datepick({showOn: 'button', yearRange: '-60:+60',
...
<div class="column span-24 last">
	<input type="submit" value="<?=lang('SAVE')?>" class="button" id="submit" name="submit"> <input type="reset" value="<?=lang('CANCEL')?>" class="button" onclick="window.location='<?=site_url(get_currcontroller().'/journal/'.$doc_rid.'/mrid/'.get_curririd()) ?>';" id="reset" name="reset">
</div>
...
$(document).ready(
		function(){
			$('#c_type_credit').change(function(){
				...
				$.ajax({
					type: 'POST',
					url: "<?=site_url(get_currcontroller()."/gccontr/go/mrid/".get_curririd())?>",
					data: query_string,
			...
			$('#c_type_debet').change(function(){
				...
				$.ajax({
					type: 'POST',
					url: "<?=site_url(get_currcontroller()."/gdcontr/go/mrid/".get_curririd())?>",
					data: query_string,
					success: function(html){
