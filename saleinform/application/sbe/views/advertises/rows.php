...
<div class="column span-11 lang">
	<?=form_input('f_sum', set_value('f_sum', ''), 'id="f_sum" class="text" style="width:50px;"')?>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
function removerow(frid){
	...
	$.ajax({
		type: "POST",
		url: "<?=site_url(get_currcontroller()."/removerow/go/mrid/".get_curririd())?>",
		data: dataStr,
...
$(document).ready(
		function(){
			function addrow(){
				...
				$.ajax({
					type: "POST",
					url: "<?=site_url(get_currcontroller()."/addrow/go/mrid/".get_curririd())?>",
					data: dataStr,
					success: function(msg){
						$('#_filials_rid').val('');
						$('#f_sum').val('');
			...
			function allfilials(){
				...
				$.ajax({
					type: "POST",
					url: "<?=site_url(get_currcontroller()."/allfilials/go/mrid/".get_curririd())?>",
					data: {},
					success: function(msg){
						$('#rows_body').html(msg);
					}
			...
			function prop(){
				...
				$.ajax({
					type: "POST",
					url: "<?=site_url(get_currcontroller()."/prop/go/mrid/".get_curririd())?>",
					data: dataStr,
					success: function(msg){
						$('#rows_body').html(msg);
					}
