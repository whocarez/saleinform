...
<?}?>
<?= form_open(get_currcontroller()."/vremove/go/mrid/".get_curririd(), array('id'=>'vgrid_'.$orid, 'autocomplete'=>'off'))?>
<table cellpadding="0" cellspacing="0">
	<tr>
		<td colspan="<?=count($fields)+2?>" style="text-align: right;background-color: #f6f6f6;">
			<div class="column span-12" style="text-align: left;padding-top:3px;">
				<?if(in_array('N', $tools)) { ?>
					<input type="button" value="<?=lang('NEW_TOOL')?>" class="button" name="create_btn_<?=$orid?>" onclick="window.location='<?=site_url(get_currcontroller()."/vcreate/go/mrid/".get_curririd());?>'">
				<? } ?>
				<?if(in_array('R', $tools)) { ?>
	...
	<tr>
		...
		<td class="gridHeader" width="<?=element('colwidth', $field, '')?>" style="white-space: nowrap;">
			<? if($field['sort']) { ?>
			<?=anchor(get_currcontroller().'/vsort/'.$key.'/mrid/'.get_curririd(), $field['label'], "title=\"".$field['label']."\"")?>
			&nbsp;&nbsp;&nbsp;
