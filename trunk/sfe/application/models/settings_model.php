...
class Settings_model extends Model
{
	...
	public function GetSettings($countryRID, $addcurrencyRID = null, $regionRID = null, $cityRID = null){
		if($cityRID){
	...
	public function getCountryInfo($rid){
		$this->db->select('_countries.*, _currency.name as curr_name');
		$this->db->from('_countries');
		$this->db->join('_currency', '_currency.rid=_countries._currency_rid');
		$this->db->where(array('_countries.archive'=>'0', '_countries.rid'=>$rid));
		$this->db->orderby('name ASC');
		$this->db->limit(1);
		$query = $this->db->get();
