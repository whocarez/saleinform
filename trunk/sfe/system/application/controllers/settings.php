...
class Settings extends Controller 
{
	...
	public function index()
	{
		...
		$this->objectsArr['settings1_area_obj'] = $this->settings_module->RenderSettingsArea();
		$this->objectsArr['keywords_area_obj'] = $this->keywords_module->RenderKeywordsArea($this->objectsArr['settings_area_obj']);
		$this->load->view('layouts/settings.php', $this->objectsArr);
	}
	
	public function _remap($methodNAME)
	{
		if($methodNAME=='aj_change_country')
		{
			 if($countryRID = $this->uri->segment(3)) $this->_AJAX_change_country();
		}
		else if($methodNAME=='aj_change_cities')
		{
			 if($regionRID = $this->uri->segment(3)) $this->_AJAX_change_cities();
		}
		else $this->index();
	}
	...
	private function _AJAX_change_country()
	{
		...
		$this->settings_module->SetCountryRid($countryRID);
		echo $this->settings_module->RenderSetSettingsArea();
	}

	private function _AJAX_change_cities()
	{
		$regionRID = $this->uri->segment(3);
		echo $this->settings_module->RenderCitiesList($regionRID);
	}
